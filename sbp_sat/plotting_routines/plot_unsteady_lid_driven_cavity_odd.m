...
filename_dvm = '../DVM/unsteady_lid_driven_cavity/result_DVM_20.txt';
result_mom = cell(length(M_values));% result from moments
result_dvm = dlmread(filename_dvm,'\t');

for i = 1 : length(M_values)
    filename_moments{i} = strcat('../unsteady_lid_driven_cavity_odd/','result_M',num2str(M_values(i)),'.txt');
...
subplot(1,2,1)
surf(X,Y,reshape(result_dvm(ID_rho,:),grid_points,grid_points))
...
subplot(1,2,2)
surf(X,Y,reshape(result_mom{6}(ID_rho,:),grid_points,grid_points))
title('Moment method');
xlabel('X')
ylabel('Y')
zlabel('\sigma_{xy}')
...
%% residual variation
filename = '../unsteady_lid_driven_cavity_odd/residual_M13.txt';
